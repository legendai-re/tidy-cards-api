...
module.exports = function updatePassword (params, currentUser, callback) {
  ...
  if (!requiredParamsOk(params)) { return callback(new m.ApiResponse('some required parameters was not provided', 400)) }
  ...
  m.User.findOne({_id: currentUser._id}).select('+local.password').exec(function (err, currentUser) {
    ...
    if (currentUser.local && currentUser.local.active) {
      ...
      if (!passwordsMatch(currentUser.local.password, params.password)) { return callback(new m.ApiResponse('given password do not match with currentUser password', 401)) }
