...
module.exports = function create (user, collectionId, displayMode, description, title, itemType, itemContent, url, callback) {
  ...
  if (!collectionId) { return callback(new m.ApiResponse('you must define a collectionId', 400)) }

  // if a specific displayMode is defined and is OK, update the item
  if (displayMode && displayModeOk(displayMode)) { item.displayMode = displayMode }
  ...
  m.Collection.findById(collectionId).populate('_collaborators').exec(function (err, collection) {
    ...
    if (!collection.haveEditRights(user)) { return callback(new m.ApiResponse('only the author or collab of the collection can add item', 401)) }
  ...
  function manageItemWithContent (collection) {
    itemContentController.checkItemContent(user, itemType, itemContent, function (err, content) {
      if (err) { logger.error(err); return callback(new m.ApiResponse(err, 500)) }
      if (!content && !item.description) { return callback(new m.ApiResponse('you must add a description if there is no url', 400)) }
      if (content) {
  ...
  function manageItemWithNoContentYet (collection) {
    ...
    itemContentController.create(user, itemType, url, function (apiResponse) {
      if (apiResponse.err) { logger.error(apiResponse.err); return callback(new m.ApiResponse(apiResponse.err, 500)) }

      let itemContent = apiResponse.data.itemContent

      if (!itemContent && !item.description) { return callback(new m.ApiResponse('you must add a description if there is no url', 400)) }

      if (itemContent) {
        if (!item.title) { item.title = getTitleFromContent(itemType, itemContent) }
