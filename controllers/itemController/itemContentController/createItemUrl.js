    ...
    if (error) {
      ...
      if (img !== null && img !== '' && img !== undefined) {
        if (!(img.substring(0, 7) === 'http://' || img.substring(0, 8) === 'https://')) {
          if (img[0] === '/' && img[1] === '/') { img = 'http:' + img} else if (img[0] === '/') { img = 'http://' + host + img} else { img = 'http://' + host + '/' + img}
        }
        itemUrl.image = img
      }

      itemUrl.title = $('meta[property="og:title"]').attr('content')
      if (itemUrl.title === '' || itemUrl.title === null || itemUrl.title === undefined) { itemUrl.title = $('title').html()}

      itemUrl.description = $('meta[property="og:description"]').attr('content')
      if (itemUrl.description === '' || itemUrl.description === null || itemUrl.description === undefined) { itemUrl.description = $('meta[name="description"]').attr('content')}

      itemUrl.author = $('meta[name="author"]').attr('content')
      if (itemUrl.author === '' || itemUrl.author === null || itemUrl.author === undefined) { itemUrl.author = $('meta[property="article:author"]').attr('content')}
