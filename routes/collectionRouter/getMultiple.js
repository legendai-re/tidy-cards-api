module.exports = function getMultiple (req, res) {
  ...
  if (rq._author && rq.custom_sort) {
    ...
    models.CustomSort.findOne({ _user: rq._author, type: sortTypes.MY_COLLECTIONS.id }, { ids: { $slice: [skip, limit] } }, function (err, customSort) {
      if (err || !customSort) { return res.status(500).json({error: 'cannot find customSort related to user: ' + rq._author}) }
    ...
    getQueryFiler(rq, req, function (filterObj) {
      ...
      if (rq.skip) { q.skip(parseInt(rq.skip)) }
      if (rq.limit) { q.limit(parseInt(rq.limit)) }
      q.exec(function (err, collections) {
        if (err) { console.log(err); res.sendStatus(500); return }
        if (req.user) {
          addIsStarred(req.user, collections, function (collectionsResult) {
  ...
  function getQueryFiler (rq, req, callback) {
    ...
    if (!rq.search) { filterObj.depth = 0 }
    if (rq.isFeatured) { filterObj.isFeatured = true }
    if (rq.isOnDiscover) { filterObj.isOnDiscover = true }
    ...
    if (rq.search) {
      ...
      if (req.user && req.user.isGranted('ROLE_USER')) { filterObj.$or = [{visibility: visibility.PUBLIC.id}, {_author: req.user._id}] } else { filterObj.visibility = visibility.PUBLIC.id }
      callback(filterObj)
    } else {
