module.exports = function put (req, res) {
  ...
  models.Item.findById(req.params.item_id).populate({path: '_collection', populate: {path: '_collaborators'}}).populate('_author').exec(function (err, item) {
    ...
    if (req.body.updatePosition && typeof req.body.position !== 'undefined') {
      ...
      if (req.body.displayMode && displayModeOk(req.body.displayMode)) { item.displayMode = req.body.displayMode }

      if (req.body.description || req.body.description === '') { item.description = req.body.description }

      if (req.body.title) { item.title = req.body.title }

      if (!typeOk(req.body.type)) { return res.status(400).send({ error: 'bad item type' }) }
      ...
      itemContentController.checkItemContent(req.user, req.body.type, req.body._content, function (err, content) {
        ...
        if (content) { item._content = content._id }
