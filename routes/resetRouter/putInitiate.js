module.exports = function putInitiate (req, res) {
  ...
  if (!req.body.user_id) {
    ...
    models.User.findOne({_id: req.body.user_id}).select('+local.resetToken').exec(function (err, user) {
      ...
      if (!isAbleToinitiateReset(user)) { return sendResponse('request done less than 24 hours ago or no local strategy or emailnot confirmed', false) }
  ...
  function isAbleToinitiateReset (user) {
    if (!user.local.active || !user.emailConfirmed) { return false }
