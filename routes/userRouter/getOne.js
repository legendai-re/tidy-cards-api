module.exports = function getOne (req, res) {
  ...
  if (mongodbid.isMongoId(req.params.user_id)) { q = models.User.findById(req.params.user_id) } else if (isEmail(req.params.user_id)) { q = models.User.findOne({email: noCaseUsernameRegex}) } else { q = models.User.findOne({username: noCaseUsernameRegex}) }
